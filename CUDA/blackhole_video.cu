...
{
    float x, y, z;
    __host__ __device__ vec3(float _x = 0.f, float _y = 0.f, float _z = 0.f) : x(_x), y(_y), z(_z) {}
    __host__ __device__ float length() const { return sqrtf(x * x + y * y + z * z); }
    __host__ __device__ float lengthSq() const { return x * x + y * y + z * z; }
...
__device__ vec3 getDiskEmissionColor(const vec3 &pos, const vec3 &view_dir_at_disk)
{
...
__device__ vec3 backgroundStars(const vec3 &dir, curandState *localState)
{
...
__device__ vec3 geodesic_acceleration(const vec3 &pos, const vec3 &dir)
{
...
__device__ void integrateRK4(vec3 &pos, vec3 &dir, float dt)
{
    vec3 k1_pos = dir;
...
__device__ bool intersectDisk(const vec3 &p1, const vec3 &p2, vec3 &intersection_point)
{
    if (p1.z * p2.z >= 0)
        return false;
    float dz = p2.z - p1.z;
...
__device__ vec3 traceRay(vec3 ray_pos, vec3 ray_dir, curandState *localState)
{
    ...
    for (int step = 0; step < MAX_STEPS; ++step)
    {
        ...
        vec3 prev_dir = current_dir; // Store previous direction too
        ...
        {
            vec3 view_direction_at_disk = -prev_dir;
            return getDiskEmissionColor(intersection_pt, view_direction_at_disk);
        }
        if (r_sq > MAX_DIST * MAX_DIST)
            return backgroundStars(current_dir, localState);
    }
    return backgroundStars(current_dir, localState);
}

__global__ void renderKernel(vec3 *output_buffer, int width, int height, vec3 cam_pos, vec3 cam_look_at, vec3 cam_up, float fov, unsigned long long seed)
{
    ...
    curand_init(seed, y * width + x, 0, &localState);
    ...
    vec3 color = traceRay(cam_pos, ray_dir, &localState);
...
void saveImage(const std::string &filename, const std::vector<vec3> &buffer, int width, int height, int jpg_quality = 95)
{
    ...
    {
        std::cerr << "Error: Unsupported image format extension: '" << ext << "'. Saving fallback." << std::endl;
        success = stbi_write_png("output_fallback.png", width, height, num_components, image_data.data(), stride_in_bytes);
        if (!success)
        {
            std::cerr << "Error: Fallback PNG save failed." << std::endl;
            return;
        }
    }
    if (success)
...
int main()
{
    ...
    const size_t buffer_size = width * height * sizeof(vec3);
    const float fov = 75.0f;

    const int NUM_FRAMES = 120;
    const std::string OUTPUT_FOLDER = "blackhole_frames";
    ...
    std::cout << "Initializing Black Hole Animation Renderer..." << std::endl;
    ...
    vec3 *d_buffer = nullptr;
    cudaCheckError(cudaMalloc(&d_buffer, buffer_size));
    ...
    for (int frame = 0; frame < NUM_FRAMES; ++frame)
    {
        ...
        unsigned long long seed = 12345ULL + frame;
